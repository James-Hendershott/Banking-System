...
function addTableData() {
    ...
    Promise.all(userInsertPromises)
        .then(() => {
            console.log('All users added. Adding accounts...');
            // Step 3: Add accounts after users
            const accountData = [
            ...
            accountData.forEach(({ username, checking, savings }) => {
                const userIdQuery = `SELECT user_id FROM users WHERE username = ?`;
                con.query(userIdQuery, [username], (err, results) => {
                    if (err) {
                        console.error(`Error fetching user_id for '${username}':`, err.message);
                        return;
                    }
                    ...
                    if (results.length > 0) {
                        ...
                        `;
                        con.query(accountSql, [userId, checking, userId, savings], (err) => {
                            if (err) {
                                console.error(`Error adding accounts for '${username}':`, err.message);
                                return;
                            }
                            console.log(
                                `Accounts added for '${username}' (user_id: ${userId}): Checking $${checking}, Savings $${savings}.`
                            );
                        });
                    } else {
