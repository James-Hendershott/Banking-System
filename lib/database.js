...
function addDummyDataToDatabase() {
    ...
    // Add roles using stored procedure
    const roles = ['regular', 'admin', 'employee'];
    roles.forEach((role) => {
        con.query('CALL add_user_role(?)', [role], (err) => {
            if (err) {
    ...
    // Add admin user using stored procedure
    ...
    con.query(
        ...
        (err) => {
            if (err) {
                ...
                con.query(`CALL change_user_type('admin', 'admin')`, (err) => {
                    if (err) {
    ...
    // Add a dummy customer using stored procedure
    con.query(
        `CALL register_user('customer1', 'customerpassword', 'John', 'Doe', 'johndoe@example.com', @result)`,
        (err) => {
            if (err) {
                console.error('Error adding customer user:', err);
            } else {
                console.log('Customer user added.');

                // Add checking and savings accounts for customer
                const accounts = [
                    { type: 'Checking', balance: 1000.00 },
                    { type: 'Savings', balance: 5000.00 },
                ];
                accounts.forEach(({ type, balance }) => {
                    con.query(
                        `CALL add_bank_account((SELECT user_id FROM users WHERE username = 'customer1'), ?, ?)`,
                        [type, balance],
                        (err) => {
                            if (err) {
                                console.error(`Error adding ${type} account for customer:`, err);
                            } else {
                                console.log(`${type} account with balance $${balance} added for customer.`);
                            }
                        }
                    );
                });
    ...
    // Add a dummy employee using stored procedure
    con.query(
        `CALL register_user('employee1', 'employeepassword', 'Jane', 'Smith', 'janesmith@example.com', @result)`,
        (err) => {
            if (err) {
                console.error('Error adding employee user:', err);
            } else {
                console.log('Employee user added.');
            }
        }
    );
}
