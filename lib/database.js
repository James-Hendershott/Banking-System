...
function addTableData() {
    ...
    roles.forEach((role) => {
        ...
        con.query(sql, [role], (err) => {
            ...
            console.log(`Added '${role}' role to user_roles`);
        });
    });

    // Add admin user
    const adminSalt = generateSalt();
    const adminPassword = hashPasswordWithSalt(adminSalt, 'adminpassword');
    const adminSql = `
        CALL register_user(
            'admin',
            ?, -- Hashed password
    ...
    `;
    con.query(adminSql, [adminPassword, adminSalt], (err) => {
        if (err) {
    ...
    const customerPassword = hashPasswordWithSalt(customerSalt, 'customerpassword');
    const customerSql = `
        CALL register_user(
            'customer',
            ?, -- Hashed password
    ...
    `;
    con.query(customerSql, [customerPassword, customerSalt], (err) => {
        if (err) {
    ...
    // Add employee user
    const employeeSalt = generateSalt();
    const employeePassword = hashPasswordWithSalt(employeeSalt, 'employeepassword');
    const employeeSql = `
        CALL register_user(
            'employee',
            ?, -- Hashed password
    ...
    `;
    con.query(employeeSql, [employeePassword, employeeSalt], (err) => {
        if (err) {
