...
function addTableData() {
    ...
    roles.forEach((role) => {
        const sql = `INSERT IGNORE INTO user_roles (type) VALUES (?)`;
    ...
    // Insert users and assign roles if they do not already exist
    users.forEach((user) => {
        const userSql = `
            INSERT INTO users (username, hashed_password, salt, first_name, last_name, email, user_role_id)
            SELECT ?, ?, ?, ?, ?, ?, id
    ...
    // Add accounts for the customer if they do not already exist
    const accounts = [
        { username: 'customer', account_type: 'Checking', balance: 1000 },
        { username: 'customer', account_type: 'Savings', balance: 5000 },
    ];
    ...
    accounts.forEach((account) => {
        ...
        con.query(
            accountSql,
            [account.account_type, account.balance, account.username, account.username, account.account_type],
            (err, results) => {
                if (err) {
                    console.log(
                        `Error adding '${account.account_type}' account for user '${account.username}':`,
                        err.message
                    );
                    throw err;
                }
                if (results.affectedRows > 0) {
                    console.log(
                        `'${account.account_type}' account for user '${account.username}' added successfully.`
                    );
                } else {
                    console.log(
                        `'${account.account_type}' account for user '${account.username}' already exists. Skipping.`
                    );
                }
            }
        );
