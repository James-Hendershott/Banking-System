...
async function createStoredProcedures() {
    const procedures = [
        ...
        
        {
            ...
            sql: `
            CREATE PROCEDURE fetch_user_by_id(IN in_user_id INT)
            BEGIN
                SELECT 
                    u.username,
                    u.hashed_password,
        ...
        {
            name: 'fetch_transactions', // Retrieve transaction history for all accounts of a user
            sql: `
            ...
            BEGIN
                ...
                JOIN bank_accounts b 
                    ON t.from_account = b.account_id 
                    OR t.to_account = b.account_id
        ...
        {
            name: 'fetch_transactions_by_account', // Retrieve transactions for a specific account
            sql: `
            CREATE PROCEDURE fetch_transactions_by_account(
            ...
            BEGIN
                SELECT 
                    ...
                    t.timestamp
                FROM transactions t
                WHERE t.from_account = in_account_id OR t.to_account = in_account_id
                ORDER BY t.timestamp DESC;
            END;
            `,
        },
