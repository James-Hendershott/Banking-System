...
async function setupDatabase() {
    await query('CREATE DATABASE IF NOT EXISTS banking_system');
    await query('USE banking_system');
    console.log('Using banking_system database.');

    await dropExistingProcedures();
    await createTables();
...
async function createStoredProcedures() {
    ...
    for (const { name, sql } of procedures) {
        try {
            await query(sql);
            console.log(`Stored procedure "${name}" created.`);
        } catch (err) {
