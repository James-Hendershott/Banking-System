...
const crypto = require('crypto'); // Crypto module for generating secure data
...
async function createStoredProcedures() {
    const procedures = [
        ...
        },
        {
            ...
            BEGIN
                ...
                IF to_account_exists = 0 THEN
                    SET out_result = 3; -- Error: Destination account does not exist
                    LEAVE BEGIN;
                END IF;

                -- Verify sufficient balance in the source account
        ...
        {
            name: 'add_transaction', // Log a transaction between accounts
            sql: `
        ...
        },
        {
            ...
            BEGIN
                -- Fetch account types and balances for the specified user
...
async function addTableData() {
    ...
    for (const role of roles) {
        try {
            await query('CALL add_user_role(?)', [role]);
    ...
    for (const user of users) {
        try {
            // Call the `register_user` stored procedure to handle user creation
            const params = [
                user.username,
            ...
            const result = await query('CALL register_user(?, ?, ?, ?, ?, ?, @out_result)', params);
            ...
            if (userIdResult.length > 0) {
                userIdMap[user.username] = userIdResult[0].user_id;
                console.log(`User '${user.username}' added with ID ${userIdResult[0].user_id}.`);
            }
        } catch (err) {
